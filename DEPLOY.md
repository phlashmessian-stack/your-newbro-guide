# 🚀 NeuroBro — Инструкция по деплою и настройке
...
3. Создайте Edge Function для обработки платежей
4. ```bash
   supabase secrets set YOOKASSA_SHOP_ID=xxx YOOKASSA_SECRET_KEY=xxx
   ```
...
3. ```bash
   supabase secrets set CP_PUBLIC_ID=xxx CP_API_SECRET=xxx
   ```

#### Robokassa
1. https://robokassa.com
2. Получите `merchantLogin` и пароли
3. ```bash
   supabase secrets set ROBO_LOGIN=xxx ROBO_PASS1=xxx ROBO_PASS2=xxx
   ```
